...
def processFeedback(feedback):
    ...
    if feedback.event_type == InteractiveMarkerFeedback.MENU_SELECT:
        if feedback.menu_entry_id==1:
            ...
            listener.waitForTransform('/ee_link','/1_Link',rospy.Time(), rospy.Duration(1.0))
            (trans,rot)=listener.lookupTransform('1_Link','ee_link',rospy.Time())
            print (trans,rot)
...
if __name__=="__main__":
	...
	int_marker.description = "Upper arm ik Control"
	int_marker.scale=1


	listener.waitForTransform('/ee_link','/1_Link',rospy.Time(), rospy.Duration(1.0))
	print (listener.frameExists('1_Link'))
	print (listener.frameExists('ee_link'))
	(trans,rot)=listener.lookupTransform('1_Link','ee_link',rospy.Time())
	print (trans,rot)
	...
	rotate_control.orientation.w=0
	rotate_control.orientation.x=0
	rotate_control.orientation.y=0
	rotate_control.orientation.z=0
	rotate_control.name = "moving"
	rotate_control.interaction_mode = InteractiveMarkerControl.MOVE_ROTATE_3D
	...
	menu_handler.insert( "Move to end effector", callback=processFeedback )
