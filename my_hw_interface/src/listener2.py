...
def callBackFunc(message):
	...
	rev_data.joint1 = int(((message.joint1 - prev_arr[0]) * GEAR_RATIO * STEPS_PER_REVOLUTION) / (2 * PI))
	rev_data.joint2 = int(((message.joint2 - prev_arr[1]) * GEAR_RATIO * STEPS_PER_REVOLUTION) / (2 * PI))
	rev_data.joint3 = int(((message.joint3 - prev_arr[2]) * GEAR_RATIO * STEPS_PER_REVOLUTION) / (2 * PI))
	rev_data.joint4 = int(((message.joint4 - prev_arr[3]) * GEAR_RATIO * STEPS_PER_REVOLUTION) / (2 * PI))
	rev_data.joint5 = int(((message.joint5 - prev_arr[4]) * GEAR_RATIO * STEPS_PER_REVOLUTION) / (2 * PI))
...
subsciber1 = rospy.Subscriber(moveitTopicName, rad_msg, callBackFunc, queue_size = None)
